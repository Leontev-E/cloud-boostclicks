...
impl<'d> StoragesService<'d> {
    ...
    pub async fn create(
        ...
        user: &AuthUser,
    ) -> CloudBoostclicksResult<Storage> {
        ...
        if let Ok(_) = self
            ...
            .await
        {
            return Err(CloudBoostclicksError::StorageNameConflict);
        }
        ...
        let access_schema = GrantAccess::new(user.identifier.clone(), AccessType::A);
        let result = self
            .access_repo
    ...
    pub async fn list(&self, user: &AuthUser) -> CloudBoostclicksResult<Vec<StorageWithInfo>> {
        self.repo.list_by_user_id(user.id).await
    }

    pub async fn get(&self, id: Uuid, user: &AuthUser) -> CloudBoostclicksResult<Storage> {
        check_access(&self.access_repo, user.id, id, &AccessType::R).await?;

        self.repo.get_by_id(id).await
    }

    pub async fn delete(&self, id: Uuid, user: &AuthUser) -> CloudBoostclicksResult<()> {
    ...
    pub async fn grant_access(
        ...
        if in_schema.user_email == user.identifier {
            return Err(CloudBoostclicksError::CannotManageAccessOfYourself);
        }
