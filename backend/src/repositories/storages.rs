...
impl<'d> StoragesRepository<'d> {
    ...
    pub async fn create(&self, in_obj: InStorage) -> CloudBoostclicksResult<Storage> {
        let id = Uuid::new_v4();
        ...
        .map_err(|e| match e {
            sqlx::Error::Database(dbe) if dbe.is_foreign_key_violation() => {
                CloudBoostclicksError::UserWasRemoved
            }
    ...
    pub async fn list_by_user_id(&self, user_id: Uuid) -> CloudBoostclicksResult<Vec<StorageWithInfo>> {
        sqlx::query_as(
            format!(
                "
    ...
    pub async fn get_by_id(&self, id: Uuid) -> CloudBoostclicksResult<Storage> {
    ...
    pub async fn get_by_file_id(&self, file_id: Uuid) -> CloudBoostclicksResult<Storage> {
        sqlx::query_as(
    ...
    pub async fn delete_storage(&self, storage_id: Uuid) -> CloudBoostclicksResult<()> {
