...
impl<'d> FilesRepository<'d> {
    ...
    pub async fn create_file(&self, in_obj: InFile) -> CloudBoostclicksResult<File> {
        self._create_file(in_obj, false).await
    }

    pub async fn create_folder(&self, in_obj: InFile) -> CloudBoostclicksResult<File> {
        self._create_file(in_obj, true).await
    }

    async fn _create_file(&self, in_obj: InFile, is_uploaded: bool) -> CloudBoostclicksResult<File> {
        let id = Uuid::new_v4();
        ...
        .map_err(|e| match e {
            sqlx::Error::Database(dbe) if dbe.is_foreign_key_violation() => {
                CloudBoostclicksError::DoesNotExist("такое облако не существует".to_string())
            }
            sqlx::Error::Database(dbe) if dbe.is_unique_violation() => {
                CloudBoostclicksError::AlreadyExists("файл с таким именем уже существует".to_string())
            }
            _ => {
                tracing::error!("{e}");
    ...
    pub async fn create_file_anyway(&self, in_obj: InFile) -> CloudBoostclicksResult<File> {
        let id = Uuid::new_v4();
    ...
    pub async fn get_file_by_path(&self, path: &str, storage_id: Uuid) -> CloudBoostclicksResult<File> {
        sqlx::query_as(
    ...
    pub async fn delete_with_folders(&self, id: Uuid) -> CloudBoostclicksResult<()> {
